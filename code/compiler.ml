...
module Reader : READER = struct
  ...
  let rec pair_from_list = function
  | [x] -> ScmPair(x, ScmNil)
  | list -> ScmPair((List.hd list), (pair_from_list (List.tl list)))
  ...
  and nt_string str =
    ...
    nt1 str
  and nt_vector str =
    let nt1 = make_skipped_star nt_sexpr in
    let nt1 = star nt1 in
    let nt1 = caten (char '#') (caten (char '(') (caten nt1 (char ')'))) in
    let nt1 = pack nt1 (function (_, (_, (sexprs, _))) -> ScmVector(sexprs)) in
    nt1 str
  and nt_list str =
    let nt1 = make_skipped_star nt_sexpr in
    let nt1 = star nt1 in
    let nt1 = caten (char '(') (caten nt1 (char ')')) in
    let nt1 = pack nt1 (function (_, (sexprs, _)) -> pair_from_list sexprs) in
    nt1 str
...
module Tag_Parser : TAG_PARSER = struct
                ...
                ScmPair (expr,
                         ScmPair (dit,
                                  ScmPair (ScmBoolean false,
                                           ScmNil))));;
  let rec names_and_values = function
  | ScmNil -> ([], [])
  | ScmPair (ScmPair (n, ScmPair (v, ScmNil)), ribs) ->
    let (ns, vs) = names_and_values ribs in
    (n :: ns, v :: vs)
  | _ -> raise (X_syntax "Invalid let-rib structure");;
  
  let rec names_and_whatevers = function
  | ScmNil -> ([], [])
  | ScmPair (ScmPair (n, ScmPair ((ScmSymbol "'whatever"), ScmNil)), ribs) ->
    let (ns, ws) = names_and_whatevers ribs in
    (n :: ns, (ScmSymbol "'whatever") :: ws)
  | _ -> raise (X_syntax "Invalid let-rib structure");;
  ...
  | _ -> raise(X_syntax "malformed cond-ribs");;
    ...
    (* | ScmPair (ScmPair(ScmSymbol "else", exprs) ,_) -> ScmPair(ScmSymbol "begin", exprs) *)
    (* | ScmPair (ScmPair(test , ScmPair(ScmSymbol "=>", exprs)), rest) ->  ScmPair(ScmSymbol "begin", ScmSymbol "begin") *)
      (* (* ScmPair(ScmSymbol "let",
        ScmPair(
          ScmPair(
            ScmPair(
    ...
    (* | ScmPair (rib, rest) -> ScmPair(ScmNil, ScmNil) *)
        (* ScmPair(ScmSymbol "if", 
          ScmPair ((List.hd rib),
            ScmPair (
              ScmPair(ScmSymbol "begin", List.tl rib),
          ScmPair((macro_expand_cond_ribs rest),
          ScmNil)))) *)
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
      let (names, values) = names_and_values ribs in 
      let names_and_whatevers_ribs = List.fold_right(fun name ribs  -> ScmPair (ScmPair (name, ScmPair (ScmSymbol "'whatever", ScmNil)), ribs)) names ScmNil in
      ...
      let sets_and_exprs = List.fold_right (fun car cdr -> ScmPair (car, cdr)) make_sets exprs in
        tag_parse (ScmPair (ScmSymbol "let", ScmPair (names_and_whatevers_ribs, sets_and_exprs)))
